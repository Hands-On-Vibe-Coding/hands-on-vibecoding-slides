...
> <span class="accent">규칙은 팀/조직/회사 수준에서 강제되어야 한다.</span>
...
```
[1] 문제 정의      → 어떤 프로젝트부터 시작할지 AI에게 묻기
[2] 풀 코드 먼저   → 동작하는 전체 코드를 받아 일단 실행
[3] 재귀적 질문    → 모르는 부분을 "아하!"까지 파고들기
[4] 극도로 구체적  → shape, 중간상태, 대안까지 요구
[5] 하루 100 질문  → 질문 수에 죄책감 갖지 않기
```

- 에러·경고가 곧 <span class="accent">학습 포인트 리스트</span>
- 바닥 개념까지 내려갔다 프로젝트로 다시 올라온다
- <span class="warn">"복붙 비브 코더"가 아니다 — 반드시 코드를 읽고 이해</span>
...
> <span class="warn">AI 탭을 상시 열어두고 페어 프로그래머처럼 쓴다.</span>
